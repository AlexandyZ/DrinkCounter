(function() {
  ...
  DateRangePicker = (function() {
    ...
    DateRangePicker.prototype.showCustomDate = function() {
	  ...
	  //document.write(this.startDate()); Fri May 01 2015 00:00:00 GMT-0700 (Pacific Daylight Time);
    //begin = this.startDate().getFullYear() + (this.startDate().getMonth()+1) + this.startDate().getDate();
    //end = this.endDate().getFullYear() + (this.endDate().getMonth()+1) + this.endDate().getDate();
      return this.$select[0].selectedIndex = this.customOptionIndex;
    };

    DateRangePicker.prototype.formatDate = function(d) {
	  //get data by month, day, year
      var m = d.getMonth() + 1;
      m = m < 10 ? ("0" + m) : m;
      var da = d.getDate();
      da = da < 10 ? ("0" + da) : da;
      return "" + (m) + "/" + (da) + "/" + (d.getFullYear());
    };
